/**
 * SoulForge Canon Defaults & Utilities
 * 
 * Default Canon configurations for each AgentMode
 * and utility functions for Canon manipulation.
 */
...
export const DEFAULT_TONE: CanonTone = {
...
export const SIDEKICK_DEFAULTS: Partial<CanonData> = {
  agentMode: 'sidekick',
...
export const CHIEF_OF_STAFF_DEFAULTS: Partial<CanonData> = {
...
export const COACH_DEFAULTS: Partial<CanonData> = {
  agentMode: 'coach',
...
export function createEmptyCanon(): Partial<CanonData> {
  return {
    agentName: '',
    agentTitle: '',
    agentMode: 'sidekick',
...
export function getDefaultsForMode(mode: AgentMode): Partial<CanonData> {
  switch (mode) {
    case 'sidekick':
...
export function isCanonComplete(canon: Partial<CanonData>): boolean {
  const required: (keyof CanonData)[] = [
    'agentName',
    'agentTitle',
    'agentMode',
    'tone',
...
export function mergeWithDefaults(
  ...
  return {
    ...
    tone: { ...defaults.tone, ...partial.tone } as CanonData['tone'],
    autonomy: { ...defaults.autonomy, ...partial.autonomy } as CanonData['autonomy'],
    surprise: { ...defaults.surprise, ...partial.surprise } as CanonData['surprise'],
    output: { ...defaults.output, ...partial.output } as CanonData['output'],
    addressing: { ...defaults.addressing, ...partial.addressing } as CanonData['addressing'],
...
// ============================================================================
// Legacy Compatibility (8 Dimensions → Canon)
// ============================================================================

export interface LegacyDimensions {
  riskTolerance: number;
  approvalThreshold: number;
  heartbeatAggressiveness: number;
  strategicInitiative: number;
  dataAccessScope: number;
  communicationAuthority: number;
  uncertaintyHandling: number;
  executionScope: number;
}

export function dimensionsToCanon(dims: LegacyDimensions): Partial<CanonData> {
  // Map approval threshold
  const actionMode = 
    dims.approvalThreshold <= 2 ? 'autonomous_in_sandbox' :
    dims.approvalThreshold <= 4 ? 'execute_with_approval' :
    'recommend_only';
  
  // Map surprise appetite
  const appetite = 
    dims.strategicInitiative >= 4 ? 'high' :
    dims.strategicInitiative >= 2 ? 'medium' :
    'low';
  
  // Map cadence
  const cadence = 
    dims.heartbeatAggressiveness >= 4 ? 'daily_micro' :
    dims.heartbeatAggressiveness >= 2 ? 'weekly_deep' :
    'trigger';
  
  // Map output format
  const format = 
    dims.strategicInitiative >= 4 ? 'options_tradeoffs' :
    dims.strategicInitiative >= 2 ? 'result_plus_plan' :
    'result_only';
  
  // Map explanations
  const explanations = 
    dims.uncertaintyHandling <= 2 ? 'detailed_by_default' :
    dims.uncertaintyHandling <= 4 ? 'brief_by_default' :
    'on_request_only';
  ...
  return {
    ...
    surprise: {
      appetite,
      cadence,
      boundaries: generateSurpriseBoundaries(dims),
    },
    output: {
      format,
      explanations,
      confidenceDisplay: dims.uncertaintyHandling >= 3 ? 'calibrated' : 'low_med_high',
    },
    // Derive mode from dimensions
    agentMode: dims.executionScope >= 4 ? 'chief-of-staff' : 'sidekick',
  };
}

function getApprovalThresholdText(level: number): string {
  switch (level) {
    case 1: return 'Only critical/sensitive actions require approval.';
    case 2: return 'High-impact actions require approval.';
    case 3: return 'Significant actions require approval.';
    case 4: return 'Most actions require approval.';
    case 5: return 'ALL actions require explicit approval.';
    default: return 'Anything irreversible requires explicit approval.';
  }
}

function generateSurpriseBoundaries(dims: LegacyDimensions): string {
  const parts: string[] = [];
  
  if (dims.riskTolerance <= 2) {
    parts.push('No high-risk surprises');
  }
  if (dims.communicationAuthority <= 2) {
    parts.push('No external outreach');
  }
  if (dims.executionScope <= 2) {
    parts.push('No autonomous execution');
  }
  
  parts.push('Proposals only, no irreversible actions');
